...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, data: language }, { status: StatusCodes.OK });
  } catch (error) {
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const authzResult = await checkResourceAccess(userId, role as Role, "language", "update");
  ...
  const bodyValidationResult = validateBody(body, updateLanguageSchema);
...
export async function DELETE(
  ...
  const { id: userId, role } = session.user;
  const authzResult = await checkResourceAccess(userId, role as Role, "language", "delete");
  if (!authzResult.allowed) return authzResult.error;
